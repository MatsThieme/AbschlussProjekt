...
export class Move extends Behaviour {
    async update(gameTime: GameTime): Promise<void> {
        ...
        console.log(this.gameObject.rigidbody.torque);
        if (this.input.getButton(InputType.Trigger).down && !this.input.getButton(InputType.Trigger).clicked) this.gameObject.rigidbody.applyImpulse(new Vector2(0, 0.1), new Vector2(1, 0));
    }
}