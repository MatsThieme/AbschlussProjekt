...
export class Transform extends Component {
    ...
    public get position(): Vector2 {
        const center = (this.relativePosition.clone);
        const angle = -(this.gameObject.parent?.transform.rotation.radian || 0);
        const x = new Vector2(Math.cos(angle) * this.relativePosition.x - Math.sin(angle) * this.relativePosition.y, Math.sin(angle) * this.relativePosition.x + Math.cos(angle) * this.relativePosition.y);
