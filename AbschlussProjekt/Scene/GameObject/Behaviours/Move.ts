...
export class Move extends Behaviour {
    async update(gameTime: GameTime): Promise<void> {
        this.gameObject.rigidbody.force.add(new Vector2(this.input.getAxis(InputType.MoveHorizontal).value / 100000, this.input.getAxis(InputType.MoveVertical).value * gameTime.deltaTime / 100000));
